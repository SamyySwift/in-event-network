...
const AdminFacilitiesContent = () => {
  ...
  // Filter
  const filteredFacilities = facilities.filter(f => [f.name, f.description, f.location].join(" ").toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="space-y-8 animate-fade-in">
      ...
      {selectedEventId && <>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <CreateFacilityDialog events={adminEvents} defaultEventId={selectedEventId} isCreating={isCreating} onSubmit={form => {
          createFacility({
            name: form.name,
          ...
          <div className="flex justify-between mb-4">
            <Input placeholder="Search facilities…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="max-w-sm" />
          </div>

          {/* Facilities List */}
          <div className="space-y-4">
            {filteredFacilities.length > 0 ? filteredFacilities.map(facility => <React.Fragment key={facility.id}>
                  <FacilityCard facility={facility} isDeleting={isDeleting} onEdit={fac => {
            setEditingFacility(fac);
            setEditDialogOpen(true);
          }} onDelete={facility => deleteFacility(facility.id)} />
                </React.Fragment>) : <div className="text-center py-8">
          ...
          <EditFacilityDialog facility={editingFacility} events={adminEvents} isUpdating={isUpdating} open={editDialogOpen} onClose={() => setEditDialogOpen(false)} onSubmit={form => {
...
const AdminFacilities = () => <AdminLayout>
    <AdminEventProvider>
