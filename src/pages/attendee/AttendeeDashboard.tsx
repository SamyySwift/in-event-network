...
const AttendeeDashboard = () => {
  ...
  const { hasJoinedEvent, loading: participationLoading } = useEventParticipation();
  ...
  // Check if user has joined any event
  const hasEventAccess = currentEvent ? hasJoinedEvent(currentEvent.id) : 
    upcomingEvents && upcomingEvents.length > 0 ? hasJoinedEvent(upcomingEvents[0].id) : false;
