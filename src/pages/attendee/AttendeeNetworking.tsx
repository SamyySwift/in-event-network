...
const AttendeeNetworking = () => {
  ...
  const handleMessage = (
    ...
    // If the profile prefers WhatsApp and has a number, open WhatsApp
    if (messagingPreference === "whatsapp" && whatsappNumber) {
      const cleanNumber = whatsappNumber.replace(/[^0-9+]/g, '').replace(/^\+/, '');
      window.open(`https://wa.me/${cleanNumber}`, '_blank');
      return;
    }
    
    // Otherwise use in-app messaging
    setSelectedConversation({
      userId: profileId,
  ...
  const handleSendConversationStarter = async (
    ...
    console.log('Sending conversation starter', { targetProfileId, targetProfileName, starterMessage, messagingPreference, whatsappNumber });
    
    // If the target prefers WhatsApp and has a number, redirect to WhatsApp with the message
    if (messagingPreference === 'whatsapp' && whatsappNumber) {
      const cleanNumber = whatsappNumber.replace(/[^0-9+]/g, '').replace(/^\+/, '');
      ...
      toast({
        ...
        description: `Redirecting to WhatsApp with your message for ${targetProfileName}`,
      });
      return;
    }
                  ...
                  <div className="overflow-y-auto flex-1 space-y-2 pr-1 scrollbar-thin scrollbar-thumb-purple-300 scrollbar-track-transparent">
                    {conversationStarters[profile.id].map((starter, idx) => (
                      <button
                        ...
                        onClick={() => handleSendConversationStarter(
                          profile.id, 
                          profile.name || "Unknown", 
                          starter,
                          profile.messaging_preference,
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {profile.messaging_preference === 'whatsapp' && profile.whatsapp_number ? (
                            <>
          ...
          <div className="-mx-5 px-5 pt-3 pb-4 sticky bottom-0 bg-white/85 dark:bg-gray-900/85 backdrop-blur border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              <Button
                ...
                className={`flex-1 h-10 border-gray-200 dark:border-gray-600 ${
                  profile.messaging_preference === "whatsapp" && profile.whatsapp_number
                    ? "bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/30 border-green-200 dark:border-green-700"
                    : "bg-white/90 hover:bg-white dark:bg-gray-800/90 dark:hover:bg-gray-800"
                }`}
              >
                {profile.messaging_preference === "whatsapp" && profile.whatsapp_number ? (
                  <>
                    <svg viewBox="0 0 24 24" className="w-4 h-4 mr-2 text-green-600" fill="currentColor">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                    </svg>
                    <span className="text-green-700 dark:text-green-400">WhatsApp</span>
                  </>
                ) : (
                  <>
    ...
    <>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
          <TabsList className="grid w-full grid-cols-4 gap-0.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/70 p-0.5 shadow-sm h-auto sm:h-10 overflow-visible">
            {/* People */}
            <TabsTrigger
              value="people"
