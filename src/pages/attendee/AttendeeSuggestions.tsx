...
const AttendeeSuggestions = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const newCount =
        allSuggestions?.filter((s) => s.status === "new").length || 0;
      const reviewed =
        allSuggestions?.filter((s) => s.status === "reviewed").length || 0;
      const implemented =
        allSuggestions?.filter((s) => s.status === "implemented").length || 0;

      const ratings =
        allSuggestions?.filter((s) => s.type === "rating" && s.rating) || [];
      const averageRating =
        ratings.length > 0
      ...
      <div className="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            You need to join an event before you can submit suggestions and
            ratings.
          </p>
        </div>
      </div>
              ...
              <div className="flex flex-col items-center sm:items-end space-y-1 mt-4 sm:mt-0">
                ...
                <div>
                  <Badge variant="success" className="font-medium">
                    {Math.round(getStatusProgress())}% Progress
                  </Badge>
                </div>
              </div>
            </div>
              ...
              {suggestions.length > 0 ? (
                <div className="space-y-3 sm:space-y-4">
                  {suggestions.map((suggestion) => (
                    <Card
                      key={suggestion.id}
                      className="hover:shadow-lg rounded-2xl transition-shadow border-0 bg-white/95 backdrop-blur"
                    >
                ...
                <Card>
                  <CardContent className="py-10 text-center text-muted-foreground">
                    ...
                    <p className="text-sm sm:text-base">
                      You haven't submitted any suggestions or ratings yet.
                    </p>
                    <p className="text-xs sm:text-sm mt-1">
                      Use the tabs above to share your feedback!
                    </p>
                  </CardContent>
                </Card>
              )}
