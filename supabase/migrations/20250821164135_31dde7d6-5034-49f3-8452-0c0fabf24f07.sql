...
-- Add RLS policy for vendor_forms
CREATE POLICY "Event hosts can manage vendor forms for their events" 
ON public.vendor_forms FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM public.events e
    WHERE e.id = vendor_forms.event_id 
    AND e.host_id = auth.uid()
  )
);

CREATE POLICY "Anyone can view active vendor forms" 
