...
const AttendeePolls = () => {
  ...
  const { polls, userVotes, submitVote, isSubmitting, isLoading } =
    useAttendeePolls();

  // Check for active banner polls
  useEffect(() => {
    const bannerPoll = polls.find(
      (poll) => poll.is_active && (poll as any).display_as_banner
    );
                ...
                <div key={option.id} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {calculatePercentage(option.votes || 0, poll)}%
                    </span>
                  </div>
          ...
          <CardContent className="p-6">
            ...
            >
              <TabsList className="mb-6 bg-white/80 rounded-xl shadow-md grid grid-cols-3">
                ...
                <TabsTrigger value="voted" className="text-xs sm:text-sm">
                  Your Votes
                </TabsTrigger>
