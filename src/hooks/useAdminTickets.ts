...
const CACHE_KEY_TYPES = 'admin-ticket-types';
const CACHE_KEY_TICKETS = 'admin-event-tickets';
...
export const useAdminTickets = (eventIdOverride?: string) => {
  ...
  const { data: ticketTypes = [], isLoading: isLoadingTypes } = useQuery({
    ...
    queryFn: async (): Promise<TicketType[]> => {
      ...
      setCache(`${CACHE_KEY_TYPES}-${actualEventId}`, data || []);
      return data || [];
    },
    enabled: !!actualEventId,
    placeholderData: () => getCache<TicketType[]>(`${CACHE_KEY_TYPES}-${actualEventId}`) || [],
    ...
    queryFn: async (): Promise<EventTicket[]> => {
      ...
      if (ticketIds.length > 0) {
        const { data: formData } = await supabase
          ...
          .select(`*, ticket_form_fields (label, field_type, field_order)`)
      ...
      setCache(`${CACHE_KEY_TICKETS}-${actualEventId}`, ticketsWithDetails);
      return ticketsWithDetails;
    },
    enabled: !!actualEventId,
    placeholderData: () => getCache<EventTicket[]>(`${CACHE_KEY_TICKETS}-${actualEventId}`) || [],
    ...slowNetworkQueryOptions,
  });
