...
export const useAdminDashboard = () => {
  ...
  const { data: dashboardData, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<DashboardData> => {
      ...
      // Compute "performance score" out of 100 just from questions engagement.
      const attendeesCount = attendeesResult.count || 1; // prevent divide by zero
      const questionsCount = questionsResult.count || 0;

      // Up to 100 points for questions per attendee
      const questionsPerAttendee = questionsCount / attendeesCount;
      const questionsScore = Math.min((questionsPerAttendee / 3) * 100, 100); // 3+ questions/attendee = max score

      const performanceScore = Math.round(questionsScore);
